...
import com.fpto.patrimonio.domain.projection.PatrimonioDepreciavel;
...
public interface PatrimonioRepository
     ...
     */
    @Query("""
           select new com.fpto.patrimonio.domain.projection.PatrimonioDepreciavel(
                  p.categoria, p.valorCompra, p.dataCompra, p.conservacao, p.valorRecuperavel)
           from Patrimonio p
           where p.situacao = com.fpto.patrimonio.domain.enums.SituacaoPatrimonio.ATIVO
           """)
    ...
    @Query("""
           ...
           where r.id = :responsavelId
             and p.situacao = com.fpto.patrimonio.domain.enums.SituacaoPatrimonio.ATIVO
    ...
    @Query("""
           ...
           join fetch p.lotacao
           join fetch p.responsavel
           where p.situacao = com.fpto.patrimonio.domain.enums.SituacaoPatrimonio.BAIXADO
